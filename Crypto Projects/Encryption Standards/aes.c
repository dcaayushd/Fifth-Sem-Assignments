...
#define Nk 4           // Number of 32-bit words comprising the key (4 for 128-bit key)
#define Nr 10          // Number of rounds (10 for 128-bit key)
...
// AES encryption
void aesEncrypt(uint8_t* input, uint8_t* key, uint8_t* output) {
    uint8_t state[4][4];
    uint8_t roundKey[Nb * (Nr + 1) * 4];
